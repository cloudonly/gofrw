...
import (
	...
	"github.com/luomu/gofrw/logging/logfields"
)
...
func initializeKLog() {
	...
	klogFlags := flag.NewFlagSet("hive", flag.ExitOnError)
...
func (o LogOptions) GetLogFormat() LogFormat {
	...
	re := regexp.MustCompile(`^(text|json|json-ts|custom)$`)
	if !re.MatchString(formatOpt) {
		logrus.WithError(
			fmt.Errorf("incorrect log format configured '%s', expected 'text', 'json' or 'json-ts' or 'custom'", formatOpt),
...
func (f CustomFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	...
	if f.LogFormat == "" {
		f.LogFormat = "%time% %level% %caller% - %msg%\n"
	}
	...
	caller := ""
	if entry.HasCaller() {
		srcFunc := entry.Caller.Function
		splits := strings.SplitN(srcFunc, "/", -1)
		if len(splits) > 0 {
			function = splits[len(splits)-1]
		}
		file = path.Base(entry.Caller.File) // 只获取文件名
		line = entry.Caller.Line
		caller = fmt.Sprintf("[%s(%s:%d)]", function, file, line)
	}

	output := f.LogFormat
	output = stringReplace(output, "%time%", timestamp)
	output = stringReplace(output, "%level%", strings.ToUpper(logLevel))
	output = stringReplace(output, "%caller%", caller)
	output = stringReplace(output, "%msg%", message)

	return []byte(output), nil
}

// stringReplace 替换日志格式中的占位符
